...
void wifi_init_sta(void)
{
    ...
    if (bits & WIFI_CONNECTED_BIT)
    {
        ...
        xTaskCreate(&process_http_task, "process_http_task", 8192, NULL, 5, NULL);
    }
...
{
    char *l_buffer = malloc(MAX_HTTP_RECV_BUFFER + 1);
    if (l_buffer == NULL)
    {
    ...
    {
        l_read_len = esp_http_client_read(l_client, l_buffer, l_content_length);
        if (l_read_len <= 0)
        {
            ESP_LOGE(TAG, "Error read data");
        }
        l_buffer[l_read_len] = 0;
        ESP_LOGD(TAG, "read_len = %d", l_read_len);
    }
